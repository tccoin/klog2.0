...
class KlogDataEditor extends KlogDataMessageMixin(PolymerElement) {
    ...
    async _tokensChanged(tokens) {
        this._updateTitle(tokens);
        this._updateMeta(tokens);
        await this._updateAttachments(tokens);
    }

    async _updateAttachments(tokens) {
        this.image = {};
        this.attachments = [];
        ...
        await this._updatePlaceholders(this.attachments);
    }
    ...
    async _updatePlaceholders(attachments) {
        ...
        for (let image of attachments.filter(x => x.image)) {
            let match = this.data.attachments.find(x => x.image && x.url == image.url);
            if (match && match.entropy) {
                image.entropy = match.entropy;
            } else {
